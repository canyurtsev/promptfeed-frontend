...
with open('html/benchmarks.html', 'r', encoding='utf-8') as f:
    html = f.read()
...
html = re.sub(r'<script>.*?</script>', '<script src="js/pages/benchmarks.js" defer></script>', html, flags=re.DOTALL)

with open('html/benchmarks.html', 'w', encoding='utf-8') as f:
    f.write(html)
...
# Render Row replaces
script_content = script_content.replace('onclick="window.location.href=\'prompt-detail.html?id=${esc(promptId)}\'"', 'data-action="gotoDetail" data-id="${esc(promptId)}"')
...
with open('html/js/pages/benchmarks.js', 'w', encoding='utf-8') as f:
